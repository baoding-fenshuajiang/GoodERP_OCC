...
class TestEquipmentMulticompany(TransactionCase):
    ...
    def test_00_equipment_multicompany_user(self):
        """Test Check my_equipment_maintenance with equipment manager and user in multi company environment"""

        # Use full models
        Equipment = self.env['my_equipment_maintenance.equipment']
        MaintenanceRequest = self.env['my_equipment_maintenance.request']
        Category = self.env['my_equipment_maintenance.equipment.category']
        ResUsers = self.env['res.users']
        ResCompany = self.env['res.company']
        MaintenanceTeam = self.env['my_equipment_maintenance.team']

        # Use full reference.
        group_user = self.env.ref('base.group_user')
        group_manager = self.env.ref('my_equipment_maintenance.group_equipment_manager')
        ...
        # create a my_equipment_maintenance team for company A user
        team = MaintenanceTeam.sudo(equipment_manager).create({
            'name': 'Metrology',
            'company_id': company_a.id,
        })
        # create a my_equipment_maintenance team for company B user
        ...
        # create an my_equipment_maintenance stage BY user
        with self.assertRaises(AccessError):
            self.env['my_equipment_maintenance.stage'].sudo(user).create({
                'name': 'identify corrective my_equipment_maintenance requirements',
            })

        # Create an my_equipment_maintenance request for ( User Follower ).
        ...
        # Create an my_equipment_maintenance request for equipment_manager (Admin Follower)
        ...
        # Now here is total 1 my_equipment_maintenance request can be view by Normal User
