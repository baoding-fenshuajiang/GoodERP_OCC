...
class TestEquipment(TransactionCase):
    ...
    def setUp(self):
        ...
        self.equipment = self.env['my_equipment_maintenance.equipment']
        self.maintenance_request = self.env['my_equipment_maintenance.request']
        self.res_users = self.env['res.users']
        self.maintenance_team = self.env['my_equipment_maintenance.team']
        self.main_company = self.env.ref('base.main_company')
        res_user = self.env.ref('base.group_user')
        res_manager = self.env.ref('my_equipment_maintenance.group_equipment_manager')
    ...
    def test_10_equipment_request_category(self):
        ...
        equipment_01 = self.equipment.sudo(self.manager).create({
            ...
            'category_id': self.ref('my_equipment_maintenance.equipment_monitor'),
        ...
        # Create new my_equipment_maintenance request
        maintenance_request_01 = self.maintenance_request.sudo(self.user).create({
            ...
            'color': 7,
            'stage_id': self.ref('my_equipment_maintenance.stage_0'),
            'maintenance_team_id': self.ref('my_equipment_maintenance.equipment_team_maintenance')
        })

        # I check that maintenance_request is created or not
        assert maintenance_request_01, "Maintenance Request not created"

        # I check that Initially my_equipment_maintenance request is in the "New Request" stage
        self.assertEquals(maintenance_request_01.stage_id.id, self.ref('my_equipment_maintenance.stage_0'))

        # I check that change the maintenance_request stage on click statusbar
        maintenance_request_01.sudo(self.user).write({'stage_id': self.ref('my_equipment_maintenance.stage_1')})

        # I check that my_equipment_maintenance request is in the "In Progress" stage
        self.assertEquals(maintenance_request_01.stage_id.id, self.ref('my_equipment_maintenance.stage_1'))

    def test_20_cron(self):
        """ Check the cron creates the necessary preventive my_equipment_maintenance requests"""
        ...
        self.env['my_equipment_maintenance.equipment']._cron_generate_requests()
    ...
    def test_21_cron(self):
        """ Check the creation of my_equipment_maintenance requests by the cron"""
        ...
        self.assertEqual(len(tot_requests), 1, 'The cron should have generated just 1 request for the High Maintenance Monitor.')
        self.assertEqual(tot_requests.maintenance_team_id.id, team_test.id, 'The my_equipment_maintenance team should be the same as equipment one')
        self.assertEqual(tot_requests.duration, 3.0, 'Equipement my_equipment_maintenance duration is not the same as the request one')
